...
void print_symbol_table(const Elf32_Sym *symtab, int count, const char *strtab, const Elf32_Ehdr *ehdr, const Elf32_Shdr *shdr, const char *shstrtab) {
    printf("\nSYMBOL TABLE:\n");
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (symtab[i].st_shndx < 0xFF00) { // SHN_LORESERVE
             if (symtab[i].st_shndx < ehdr->e_shnum) { // Correct bounds check
                sec_name = &shstrtab[shdr[symtab[i].st_shndx].sh_name];
             } else {
                sec_name = "OOB"; // Out of bounds
             }
        } else if (symtab[i].st_shndx == 0xFFF1) { // SHN_ABS
            sec_name = "ABS";
        } else if (symtab[i].st_shndx == 0x0) { // SHN_UNDEF
            sec_name = "UND";
        } else {
            sec_name = "SPC"; // Other special section
        }
